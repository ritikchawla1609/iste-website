...
export default function AdminRecruitmentClient({
  ...
  async function handleToggleDomainStatus(domainId) {
    ...
    try {
      ...
      setStatus({ type: "status-success", message: "Domain recruitment status updated." });
    } catch (error) {
      setStatus({ type: "status-error", message: error.message || "Failed to update domain status." });
    }
  }
