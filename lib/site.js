...
export async function updateEventLink(id, registrationLinkInput, googleFormLinkInput, adminId) {
  const registrationLink = registrationLinkInput ? validateUrl(registrationLinkInput, "Registration link") : "";
  const googleFormLink = googleFormLinkInput ? validateUrl(googleFormLinkInput, "Google Form Link") : null;
  if (!registrationLink && !googleFormLink) {
    throw new HttpError(400, "Either a Registration Link or a Google Form Link must be provided.");
  }
  ...
  const row = await withTransaction(async (transactionDb) => {
    await transactionDb.execute(
      "UPDATE events SET registration_link = ?, google_form_link = ?, updated_at = ? WHERE id = ?",
      [registrationLink, googleFormLink, isoNow(), id]
    );
