...
import { normalizeAbout, normalizeNotice, DEFAULT_RECRUITMENT_STATUS } from "@/lib/presentation";
...
async function validateEventPayload(payload, existing = null) {
  ...
  const registrationLink = payload?.registrationLink ? validateUrl(payload.registrationLink, "Registration link") : "";
  const googleFormLink = payload?.googleFormLink ? validateUrl(payload.googleFormLink, "Google Form Link") : null;
  const registrationFee = sanitizeText(payload?.registrationFee, "Free");
  if (!registrationLink && !googleFormLink) {
    throw new HttpError(400, "Either a Registration Link or a Google Form Link must be provided.");
  }
...
export function getHardcodedTechnicia2026() {
  return {
    id: 2026,
    name: "Technicia 2026",
    category: "Tech Fest",
    ...
    prizes: "INR 1,00,000+",
    description: "A flagship ISTE-CUSC tech fest featuring hackathons, CUMUN, Capture The Flag, ideathons, workshops, and interactive technical events.",
    contactName: "ISTE-CUSC",
    contactEmail: "iste@cumail.in",
    status: "published",
    posterPath: "brand/iste-logo.jpg",
    minTeamSize: 1,
    maxTeamSize: 4,
...
export async function getPublicSiteData() {
  try {
    ...
    try {
      ...
      console.warn("Failed to fetch events from database, defaulting to empty:", e);
    }

    let recruitmentRows = [];
    try {
      recruitmentRows = await database.many(
        `
      ...
      console.warn("Failed to fetch recruitments from database, defaulting to empty:", e);
    }

    let domainStatus = { ...DEFAULT_RECRUITMENT_STATUS };
    try {
      domainStatus = {
        ...domainStatus,
        ...(await getSiteContentRecord(database, "recruitment_status", domainStatus))
      };
    } catch (e) {
      console.warn("Failed to fetch recruitment status from database, using default active status:", e);
    }

    const events = eventRows.map(serializeEvent);
    const hasTechnicia = events.some(
      (e) => e.name && e.name.toLowerCase().includes("technicia") && e.eventDate.startsWith("2026")
    );
    if (!hasTechnicia) {
      events.push(getHardcodedTechnicia2026());
    }
    ...
    console.error("Critical error in getPublicSiteData: falling back to fully hardcoded static data:", error);
    return {
      notice: {
        detailText: "Welcome to ISTE Student Chapter! Empowering students through technical excellence.",
        link: "#",
        active: true
      },
      about: {
        visionTitle: "Why Join ISTE?",
        visionItems: [
          "Hands-on workshops & technical seminars",
          "National level competitions and hackathons",
          "Networking opportunities with industry mentors"
        ],
        focusCards: [
          { title: "Applied Learning", text: "Bridge academic knowledge with real-world exposure through three focus pillars." },
          { title: "Chapter Experiences", text: "Join a vibrant student community dedicated to engineering education." },
          { title: "Career Readiness", text: "Enhance leadership and technical skills for the industry." }
        ]
      },
      events: [getHardcodedTechnicia2026()],
      recruitments: [
        {
          id: 1,
          ...
          deadline: "2026-09-30",
          applicationLink: "#",
          description: "Register for ISTE membership to access hands-on technical learning, leadership opportunities, workshops, competitions, and professional networking.",
          contactName: "ISTE-CUSC",
          contactEmail: "iste@cumail.in",
          status: "published"
        }
      ],
      domainStatus: { ...DEFAULT_RECRUITMENT_STATUS }
    };
  }
}
...
export async function getPublicPastEvents() {
  try {
    ...
    console.error("Failed to fetch public past events from database: returning empty array fallback:", error);
    return [];
  }
}
...
export async function createPastEvent(payload, adminId) {
  const name = sanitizeText(payload?.name);
  const category = sanitizeText(payload?.category) || "Event";
  const eventDate = validateDate(payload?.eventDate, "Event date");
  const description = sanitizeText(payload?.description);
  const winners = sanitizeText(payload?.winners);

  // Convert and save base64 data URLs
  const imagesDataUrls = payload?.imagesDataUrls || [];
  const uploadedPaths = [];
  for (const dataUrl of imagesDataUrls) {
    const saved = await saveDataUrlImage(dataUrl, "past-event");
    if (saved) {
      uploadedPaths.push(saved);
    }
  }

  const payloadImagePaths = Array.isArray(payload?.imagePaths) ? payload.imagePaths : [];
  const combinedImagePaths = [...payloadImagePaths, ...uploadedPaths].filter(Boolean);
  const imagePaths = combinedImagePaths.join(",");
  const now = isoNow();
    ...
    const insertResult = await database.execute(
      `
        INSERT INTO past_events (
          name, category, event_date, description, winners, image_paths, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `,
      [name, category, eventDate, description, winners, imagePaths, now, now]
    );

    const inserted = await database.one("SELECT * FROM past_events WHERE id = ?", [
      insertResult.lastInsertRowid
    ]);
    if (!inserted) {
      throw new HttpError(500, "Past event was saved but could not be loaded.");
    }
...
export async function updatePastEvent(id, payload, adminId) {
  ...
  const payloadImagePaths = Array.isArray(payload?.imagePaths) ? payload.imagePaths : [];
  const combinedImagePaths = [...payloadImagePaths, ...uploadedPaths].filter(Boolean);
  const imagePathsString = combinedImagePaths.join(",");

  // Clean up files that were removed from the event
  const oldPaths = existing.image_paths ? existing.image_paths.split(",") : [];
  for (const oldPath of oldPaths) {
    if (oldPath && !combinedImagePaths.includes(oldPath)) {
      try {
        await removeStoredFile(oldPath);
      } catch (err) {
        console.error("Failed to delete removed image file:", oldPath, err);
      }
    }
  }
  ...
  const row = await withTransaction(async (transactionDb) => {
    await transactionDb.execute(
      `
        ...
        WHERE id = ?
      `,
      [name, category, eventDate, description, winners, imagePathsString, isoNow(), id]
    );
...
export async function deletePastEvent(id, adminId) {
  ...
  // Clean up files
  const paths = existing.image_paths ? existing.image_paths.split(",") : [];
  for (const filePath of paths) {
    if (filePath) {
      try {
        await removeStoredFile(filePath);
      } catch (err) {
        console.error("Failed to delete image file on event delete:", filePath, err);
      }
    }
  }
...
export async function createEvent(payload, adminId) {
  ...
  const row = await withTransaction(async (database) => {
    const insertResult = await database.execute(
      `
        INSERT INTO events (
          ...
          registration_link, google_form_link, registration_fee, prizes, description, contact_name, contact_email,
    ...
    const inserted = await database.one("SELECT * FROM events WHERE id = ?", [insertResult.lastInsertRowid]);
    if (!inserted) {
      throw new HttpError(500, "Event was saved but could not be loaded.");
    }
...
export async function createRecruitment(payload, adminId) {
  ...
  const row = await withTransaction(async (database) => {
    ...
    const inserted = await database.one("SELECT * FROM recruitments WHERE id = ?", [
      insertResult.lastInsertRowid
    ]);
    if (!inserted) {
      throw new HttpError(500, "Recruitment was saved but could not be loaded.");
    }
...
export async function getPastEventsHero() {
  try {
    const database = await getDb();
    const record = await getSiteContentRecord(database, "past_events_hero", { imagePaths: [] });
    return record;
  } catch (error) {
    console.error("Failed to fetch past events hero:", error);
    return { imagePaths: [] };
  }
}

export async function updatePastEventsHero(payload, adminId) {
  const database = await getDb();
  const existing = await getSiteContentRecord(database, "past_events_hero", { imagePaths: [] });

  const imagesDataUrls = payload?.imagesDataUrls || [];
  const uploadedPaths = [];
  for (const dataUrl of imagesDataUrls) {
    const saved = await saveDataUrlImage(dataUrl, "past-event-hero");
    if (saved) {
      uploadedPaths.push(saved);
    }
  }
  ...
  for (const oldPath of oldPaths) {
    if (oldPath && !combinedImagePaths.includes(oldPath)) {
      try {
        ...
        console.error("Failed to delete removed hero image file:", oldPath, err);
      }
    }
  }

  const newRecord = { imagePaths: combinedImagePaths };
  await withTransaction(async (transactionDb) => {
    await setSiteContentRecord(transactionDb, "past_events_hero", newRecord);
    await createAuditLog(transactionDb, adminId, "update", "site_content", "past_events_hero", {
      message: "Updated previous events page banner slideshow"
    });
  });
