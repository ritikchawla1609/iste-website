...
export async function removeStoredFile(reference) {
  ...
  let adjustedReference = reference;
  if (typeof reference === "string" && reference.startsWith("uploads/")) {
    adjustedReference = path.join("public", reference);
  }

  const target = resolveInsideRoot(ROOT_DIR, adjustedReference);
...
export function readProjectBinary(relativePath) {
  ...
  if (typeof relativePath === "string" && relativePath.startsWith("uploads/")) {
    adjustedPath = path.join("public", relativePath);
  }

  const target = resolveInsideRoot(ROOT_DIR, adjustedPath);
  if (!existsSync(target) || !statSync(target).isFile()) {
