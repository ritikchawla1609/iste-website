...
async function runSchemaMigrations(executor, { postgres = false } = {}) {
  if (postgres) {
    ...
    await runOptionalMigration(
      executor,
      "ALTER TABLE events ADD COLUMN IF NOT EXISTS registration_fee TEXT DEFAULT 'Free'"
    );
    await runOptionalMigration(
      executor,
  ...
  await runOptionalMigration(executor, "ALTER TABLE events ADD COLUMN registration_fee TEXT DEFAULT 'Free'");
