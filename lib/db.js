...
async function ensureDefaultRecords(executor) {
  ...
  if (eventCount.count === 0) {
    await executor.execute(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, ["Technicia'25", "Tech Fest", "2026-05-20", "09:00", "18:00", "Chandigarh University", "2026-05-15", "#", "Innovation Showcase", "A flagship ISTE-CUSC tech fest featuring hackathons, CUMUN, Capture The Flag, ideathons, workshops, and interactive technical events.", "ISTE-CUSC", "iste@cumail.in", "published", isoNow(), isoNow()]);
  }
...
async function initializeRemoteDatabase() {
  ...
  try {
    await database.execute("ALTER TABLE events ADD COLUMN IF NOT EXISTS google_form_link TEXT");
  } catch (error) {
    // Ignore error if column already exists
  }

  try {
    await database.execute("ALTER TABLE past_events ADD COLUMN IF NOT EXISTS category TEXT NOT NULL DEFAULT 'Event'");
  } catch (error) {
    // Ignore error if column already exists
  }
  ...
  if (!initializationPromise) {
    initializationPromise = (async () => {
      if (USE_REMOTE_DATABASE) {
        ...
        try {
          await executor.execute("ALTER TABLE events ADD COLUMN google_form_link TEXT");
        } catch (error) {
          // Ignore if column already exists
        }
        try {
          await executor.execute("ALTER TABLE past_events ADD COLUMN category TEXT DEFAULT 'Event'");
        } catch (error) {
          // Ignore if column already exists
        }
